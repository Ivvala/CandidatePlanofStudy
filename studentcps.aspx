<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="studentcps.aspx.cs" Inherits="CandidatePlanofStudy.studentcps" %>
...
<body>
 ...
 <li> <asp:LinkButton ID="linkcatalogue" runat="server" PostBackUrl="~/newcatalogue.aspx">New Catalogue</asp:LinkButton></li>
	...
	<div class="hdr clearfix">
		<div style="padding:20px 0 0 23px; font-size:22px;">Generate Draft
            </div><br />
            <br />
        <div style="margin:10px 20px">
        Select Semester<asp:DropDownList ID="DropDownsemister" runat="server" AutoPostBack="True" OnSelectedIndexChanged="DropDownsemister_SelectedIndexChanged">
            ...
            Select Major
              <asp:DropDownList ID="DropDownListmajor" runat="server" AutoPostBack="True" DataSourceID="SqlDatamajor" DataTextField="majorname" DataValueField="majorname" OnSelectedIndexChanged="DropDownListmajor_SelectedIndexChanged">
                <asp:ListItem>Select</asp:ListItem>  
 ...
 <div class="content-in">
        ...
        <asp:SqlDataSource ID="SqlDataSourcestudents" runat="server" ConflictDetection="CompareAllValues" ConnectionString="<%$ ConnectionStrings:eservicesdb %>" DeleteCommand="DELETE FROM [student] WHERE [Studentid] = @original_Studentid AND (([firstname] = @original_firstname) OR ([firstname] IS NULL AND @original_firstname IS NULL)) AND (([email] = @original_email) OR ([email] IS NULL AND @original_email IS NULL)) AND (([fullname] = @original_fullname) OR ([fullname] IS NULL AND @original_fullname IS NULL)) AND (([major] = @original_major) OR ([major] IS NULL AND @original_major IS NULL)) AND (([facultyadvisor] = @original_facultyadvisor) OR ([facultyadvisor] IS NULL AND @original_facultyadvisor IS NULL)) AND (([semister] = @original_semister) OR ([semister] IS NULL AND @original_semister IS NULL)) AND (([grade] = @original_grade) OR ([grade] IS NULL AND @original_grade IS NULL)) AND (([address] = @original_address) OR ([address] IS NULL AND @original_address IS NULL)) AND (([yearofjoining] = @original_yearofjoining) OR ([yearofjoining] IS NULL AND @original_yearofjoining IS NULL))" InsertCommand="INSERT INTO [student] ([Studentid], [firstname], [email], [fullname], [major], [facultyadvisor], [semister], [grade], [address], [yearofjoining]) VALUES (@Studentid, @firstname, @email, @fullname, @major, @facultyadvisor, @semister, @grade, @address, @yearofjoining)" OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [Studentid], [firstname], [email], [fullname], [major], [facultyadvisor], [semister], [grade], [address], [yearofjoining] FROM [student] WHERE (([major] = @major) AND ([semister] = @semister))" UpdateCommand="UPDATE [student] SET [firstname] = @firstname, [email] = @email, [fullname] = @fullname, [major] = @major, [facultyadvisor] = @facultyadvisor, [semister] = @semister, [grade] = @grade, [address] = @address, [yearofjoining] = @yearofjoining WHERE [Studentid] = @original_Studentid AND (([firstname] = @original_firstname) OR ([firstname] IS NULL AND @original_firstname IS NULL)) AND (([email] = @original_email) OR ([email] IS NULL AND @original_email IS NULL)) AND (([fullname] = @original_fullname) OR ([fullname] IS NULL AND @original_fullname IS NULL)) AND (([major] = @original_major) OR ([major] IS NULL AND @original_major IS NULL)) AND (([facultyadvisor] = @original_facultyadvisor) OR ([facultyadvisor] IS NULL AND @original_facultyadvisor IS NULL)) AND (([semister] = @original_semister) OR ([semister] IS NULL AND @original_semister IS NULL)) AND (([grade] = @original_grade) OR ([grade] IS NULL AND @original_grade IS NULL)) AND (([address] = @original_address) OR ([address] IS NULL AND @original_address IS NULL)) AND (([yearofjoining] = @original_yearofjoining) OR ([yearofjoining] IS NULL AND @original_yearofjoining IS NULL))" OnSelecting="SqlDataSourcestudents_Selecting">
            <DeleteParameters>
     ...
     <asp:GridView ID="GridView1" class="tbl1 srchresultssummary" runat="server" AutoGenerateColumns="False" DataKeyNames="Studentid" DataSourceID="SqlDataSourcestudents" OnSelectedIndexChanged="GridView1_SelectedIndexChanged">
            <Columns>
